...
class Main
{
	...
	public static void main( String args[] )
	{
		...
		while( ++escenario <= escenarios ) {
			...
			//Calcular los números  de Erdos. (TODO). 
			erdos  = dicc.get("Erdos, P.");
            erdos.setErdosNumber(0);
            erdos.setEnCola(true);
            LinkedList<Autor> nodos_vivos = new LinkedList<Autor>();
            nodos_vivos.add(erdos);

            Autor nodo_actual;
	        while(nodos_vivos.size()>0){
	            nodo_actual = nodos_vivos.poll();
	            nodo_actual.setEnCola(false);
	            int n = nodo_actual.getErdosNumber()+1;
	            for (Autor coautor : nodo_actual.getCoautores()){
	            	if (coautor.getErdosNumber()>n){
	            		coautor.setErdosNumber(n);
	            		if(!coautor.getEnCola()){
		            		nodos_vivos.add(coautor);
